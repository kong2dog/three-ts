...
class LDrawLoader extends Loader {
	...
	async preloadMaterials(url) {
		...
		for (let i = 0, l = lines.length; i < l; i++) {
			...
			if (colorLineRegex.test(line)) {
				...
				const material = this.parseColourMetaDirective(
	...
	load(url, onLoad, onProgress, onError) {
		...
		fileLoader.load(
			url,
			(text) => {
				this.processObject(text, null, url, this.rootParseScope).then(function (
					result
				) {
	...
	parse(text, path, onLoad) {
		...
		this.processObject(text, null, path, this.rootParseScope).then(function (
			result
		) {
			onLoad(result.groupObject);
		});
						...
						switch (meta) {
							...
							case '!COLOUR':
								...
								if (material) {
									...
									console.warn(
										`LDrawLoader: Error parsing material${lp.getLineNumberString()}`
									);
								}
	...
	finalizeObject(subobjectParseScope) {
		...
		if (this.smoothNormals && doSmooth) {
			smoothNormals(
				subobjectParseScope.faces,
				subobjectParseScope.lineSegments
			);
		}

		const isRoot = !parentParseScope.isFromParse;
		if (
			(this.separateObjects && !isPrimitiveType(subobjectParseScope.type)) ||
			isRoot
		) {
			...
			if (subobjectParseScope.faces.length > 0) {
				objGroup.add(
					createObject(
						...
						3,
						false,
						subobjectParseScope.totalFaces
					)
				);
			}
			...
			if (subobjectParseScope.conditionalSegments.length > 0) {
				objGroup.add(
					createObject(subobjectParseScope.conditionalSegments, 2, true)
				);
			}
			...
			if (parentParseScope.groupObject) {
				...
				subobjectParseScope.matrix.decompose(
					objGroup.position,
					objGroup.quaternion,
					objGroup.scale
				);
				...
				if (separateObjects) {
					...
					for (let i = 0, l = vertices.length; i < l; i++) {
						vertices[i] = vertices[i]
							.clone()
							.applyMatrix4(subobjectParseScope.matrix);
					}
	...
	async processObject(text, subobject, url, parentScope) {
		...
		function loadSubobject(subobject) {
			return scope.cache
				.loadData(subobject.fileName)
				.then(function (text) {
					return scope.processObject(text, subobject, url, parseScope);
				})
				.catch(function () {
					console.warn(
						`LDrawLoader: Subobject "${subobject.fileName}" could not be found.`
					);
					return null;
				});
		}
