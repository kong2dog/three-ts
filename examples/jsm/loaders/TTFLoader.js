import { FileLoader, Loader } from '../../../three/three-core/Three.js';
				...
				if (unicode !== undefined) {
					...
					glyph.path.commands.forEach(function (command) {
						...
						if (command.x !== undefined && command.y !== undefined) {
							token.o += `${round(command.x * scale)} ${round(
								command.y * scale
							)} `;
						}

						if (command.x1 !== undefined && command.y1 !== undefined) {
							token.o += `${round(command.x1 * scale)} ${round(
								command.y1 * scale
							)} `;
						}

						if (command.x2 !== undefined && command.y2 !== undefined) {
							token.o += `${round(command.x2 * scale)} ${round(
								command.y2 * scale
							)} `;
						}
					});
