			...
			if (this.workerPool.length < this.workerLimit) {
				...
				worker.onmessage = function (e) {
					...
					switch (message.type) {
						...
						default:
							console.error(
								`THREE.BasisTextureLoader: Unexpected message, "${message.type}"`
							);
					}
				};
		...
		if (basisFormat === BasisFormat.ETC1S) {
			...
			try {
				...
				for (let i = 0; i < taskConfig.levels.length; i++) {
					...
					assert(
						ok,
						`THREE.BasisTextureLoader: transcodeImage() failed for level ${level.index}.`
					);
			...
			for (let i = 0; i < taskConfig.levels.length; i++) {
				...
				assert(
					ok,
					`THREE.BasisTextureLoader: transcodeUASTCImage() failed for level ${level.index}.`
				);
